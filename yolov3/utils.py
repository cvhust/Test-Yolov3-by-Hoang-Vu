...
def image_preprocess1(image, target_size):
    ...
    image_paded = np.full(shape=(ih, iw, _), fill_value=0)
    dw, dh = (iw - nw) // 2, (ih - nh) // 2
    image_paded[dh:nh + dh, dw:nw + dw, :] = image_resized
...
def recognize_characters(plate_img):
    ...
    TYPE_LICENSE = True  # True mean short license
    # False mean long license
    ...
    # V: value
    a = plate_img.copy()
    V = cv2.split(cv2.cvtColor(a, cv2.COLOR_BGR2HSV))[2]
    T = threshold_local(V, 15, offset=10, method='gaussian')  # 29 ,15
    thresh = (V > T).astype('uint8') * 255
    thresh = cv2.bitwise_not(thresh)
    # resize the license plate region to a cannonical size
    a = imutils.resize(a, width=400)
    thresh = imutils.resize(thresh, width=400)
    # thresh = cv2.medianBlur(thresh, 5)

    # load model
    model = load_model('D:/Study/PythonPrj/pythonProject/TensorFlow-2.x-YOLOv3/recog_char_model/digits_letter_model.h5')

    # bounding and classify characters
    labels = measure.label(thresh, connectivity=2, background=0)
    # print(labels.shape)
    H = labels.shape[0]
    W = labels.shape[1]
    # initialize a charCandidates mask to hold the contours of the character candidates
    chaCandidates = np.zeros(thresh.shape, dtype='uint8')
    t = 0
    candidates = []
    ...
    for label in np.unique(labels):
        ...
        if np.count_nonzero(labelMask == 255) > 500:
            # cv2_imshow(labelMask)
            t += 1
            contours, hierachy = cv2.findContours(labelMask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
            # ensure at least one contour was found in the mask
            if len(contours) > 0:
                # cv2_imshow(labelMask)
                # grap the largest contours which corresponds to the component in the mask
                # grap the bounding box for the contour
                cnt = max(contours, key=cv2.contourArea)  # contourArea return area of contour
                (x, y, w, h) = cv2.boundingRect(cnt)  # bounding box features, x, y is top-left
                # labelMask = cv2.rectangle(labelMask, (x, y), (x + w, y + h), 12, 1)
                # cv2_imshow(labelMask)
                ...
                heightRatio = h / float(a.shape[0])  # a is copy img of plate_img

                # determine if the aspect ratio, solidity, and height of the contour pass
                # the rules tests
                keepAspectRatio = aspectRatio < 1.0 and aspectRatio > 0.1
                keepSolidity = (solidity > 0.1)
                keepHeight = heightRatio > 0.3 and heightRatio < 2.0
                # print('keepAspectration: {}'.format(keepAspectRatio))
                # print('keepSolidity: {}'.format(keepSolidity))
                # print('keepHeight: {}'.format(keepHeight))
                # size of plate_img
                X = a.shape[1]
                Y = a.shape[0]
                ...
                if keepAspectRatio and keepSolidity and keepHeight:
                    ...
                    # cv2.drawContours(chaCandidates, [hull], -1, 255, -1)
                    x1min, y1min = max(x - 1, 0), max(y - 1, 0)
                    x1max, y1max = min(x + w + 1, X), min(y + h + 1, Y)
                    x_center, y_center = x + w // 2, y + h // 2
                    candidate = np.array(labelMask[y1min:y1max, x1min:x1max])
                    # candidate = imutils.resize(candidate, width=28)
                    candidate = image_preprocess1(candidate, (28, 28))

                    # prediction
                    temp = candidate.reshape((-1, 28, 28, 1)) / 255
                    prediction = model.predict(temp)
                    cls = np.argmax(prediction)
                    if cls != 32:
                        candidates.append([cls, x_center, y_center])
    ...
    if TYPE_LICENSE:
        ...
        for candidate in candidates:
            if candidate[2] < H/2:
                ups.append(candidate)
            else:
                downs.append(candidate)
        ups.sort(key=lambda z: z[1])
        downs.sort(key=lambda z: z[1])
        for item in ups:
            string += str(all_classess[item[0]])
        for item in downs:
            string += str(all_classess[item[0]])
    else:
        candidates.sort(key=lambda z: z[1])
        for candidate in candidates:
            string += str(all_classess[candidate[0]])
...
def crop_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.6, iou_threshold=0.7):
...
def crop_image1(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES,
                score_threshold=0.6, iou_threshold=0.7):
    original_image = cv2.imread(image_path)
    original_image = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
    original_image = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
...
def detect_image(Yolo, image_path, output_path, input_size=416, show=False, CLASSES=YOLO_COCO_CLASSES, score_threshold=0.6, iou_threshold=0.7, rectangle_colors=''):
    original_image      = cv2.imread(image_path)
    original_image      = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
    original_image      = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
